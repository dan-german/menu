...
Store the menu-level dataset at `data/raw/synthetic_menus.json`, with restaurant-level cuisine metadata added by the generator from the cuisine batch that produced each menu. Write `data/raw/summary.json` with basic quality counts, including restaurant count, total item count, cuisine coverage, and item-count range per menu. Downstream item-level code should flatten menus in memory when needed rather than maintaining a separate raw item file.

### 2. Eval set
Auto-label ~100 items with reference normalized name, normalized description, category, cuisine, dietary tags, and ingredients, sampled across restaurants. These labels are generated by an API model and held out from training entirely, used only for scoring during early iteration. Ingredient references are used to score ingredient guessing via precision/recall/F1 against core ingredients. Suggestion quality (are the recommended new dishes actually good) is not scored in this eval set.
